...
func (a *accrual) RunAccrualRequester(ctx context.Context, wg *sync.WaitGroup) {
	...
	ordersFromAccrual := make(chan db.OrderStatus, 1000)
...
func (a *accrual) worker(ctx context.Context, in chan string, out chan db.OrderStatus, wg *sync.WaitGroup) {
	defer wg.Done()
...
func (a *accrual) putOrdersInDB(ctx context.Context, ordersFromAccrual chan db.OrderStatus, wg *sync.WaitGroup) {

	defer close(ordersFromAccrual)
	defer wg.Done()
	var ordersList []db.OrderStatus
	go func() {
